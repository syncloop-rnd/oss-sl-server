...
function openCloneConfigurationForm() {
  ...
  $("#CloneInput").val(("#" + loadFile).replace("#files/packages/", "").replace(".sql", ""));
  ...
  cloneItButton.onclick = function() {
    ...
    if (newLocation.trim() == "") {
     swal({
          ...
          type: "error",
          confirmButtonColor: "#f2533e" // Optional: red color for error
        });
    ...
    if (newLocationArray.length < 2) {
      swal({
          ...
          type: "error",
          confirmButtonColor: "#f2533e" // Optional: red for error dialogs
        });
    ...
    modal.style.display = "none";
    saveSQL(loadFile, "files/packages/" + newLocation + ".sql");
...
function downloadBuild() {
  ...
  var content = JSON.stringify([{
    asset: loadFile.replaceAll("files/", "").replaceAll(".sql", ""),
    type: "sql"
  }]);

  let array = loadFile.replaceAll("files/", "").replaceAll(".sql", "").split("/");
