package packages.middleware.dependency.config;
...
public final class packages.middleware.pub.server.browse.MyFlowDesign {

	static JsonObject mainflowJsonObject=null;
	static final String syncBlock=new String("sync");
	public static final void main(DataPipeline dataPipeline) throws SnippetException{
		String fqn="packages.middleware.dependency.config.packages.middleware.pub.server.browse.MyFlowDesign";
		long nanoSec=0;
		...
		try{
		  if(mainflowJsonObject==null)
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/dependency/config/packages.middleware.pub.server.browse.MyFlowDesign.flow";
			  if(mainflowJsonObject==null)
		  ...
		  FlowResolver.execute(dataPipeline,mainflowJsonObject);
			...
			new SnippetException(dataPipeline,"Failed to execute packages.middleware.pub.server.browse.MyFlowDesign", new Exception(e));
		}finally{
			...
			if(stopRecursiveLogging==null && !fqn.equalsIgnoreCase("packages.middleware.pub.service.auditLogging")){
                ...
                String nodeName = dataPipeline.getGlobalConfig("nodeName");
                auditLog.put("nodeName", nodeName);

                auditLog.put("remoteAddr", dataPipeline.getRemoteIpAddr());
                auditLog.put("userId", dataPipeline.getCurrentUserProfile().getId());
                auditLog.put("urlPath", dataPipeline.getUrlPath());
