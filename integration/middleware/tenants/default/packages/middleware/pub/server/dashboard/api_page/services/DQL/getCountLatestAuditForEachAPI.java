...
public final class getCountLatestAuditForEachAPI {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/server/dashboard/api_page/services/DQL/getCountLatestAuditForEachAPI.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute getCountLatestAuditForEachAPI", new Exception(e));
		}
	}
}
