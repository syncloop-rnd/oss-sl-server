...
public final class getAuditLogsByFqn {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/server/dashboard/api_page/services/DQL/getAuditLogsByFqn.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute getAuditLogsByFqn", new Exception(e));
		}
	}
}
