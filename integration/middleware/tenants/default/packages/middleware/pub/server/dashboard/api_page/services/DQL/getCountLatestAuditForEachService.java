package packages.middleware.pub.server.dashboard.api_page.services.DQL;
...
public final class getCountLatestAuditForEachService {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/server/dashboard/api_page/services/DQL/getCountLatestAuditForEachService.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
			      ...
			      try(JsonReader reader = Json.createReader(new FileInputStream(new File(flowRef)))) {
                    mainSqlJsonObject = reader.readObject();
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute getCountLatestAuditForEachService", new Exception(e));
		}
	}
}
