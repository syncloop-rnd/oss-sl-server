...
try {
            ...
            if (auth!=null && auth.get("awsSignature") != null) {
                ...
                AWSHeaders.build(reqHeaders, method, urlParameters, url, AccessKey, SecretKey, region, service, payload.getBytes());

            }

            Map<String, Object> response = Client.invoke(url, method, formData, reqHeaders, payload, inputStream, urlParameters, settings, sslValidation);
