...
public final class updateEnvironmentWithoutApiKey {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/environments/adapter/services/DML/updateEnvironmentWithoutApiKey.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute updateEnvironmentWithoutApiKey", new Exception(e));
		}
	}
}
