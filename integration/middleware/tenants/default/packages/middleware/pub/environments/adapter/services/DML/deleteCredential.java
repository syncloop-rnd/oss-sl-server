		...
		try{
		  if(mainSqlJsonObject==null)
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/environments/adapter/services/DML/deleteCredential.sql";
			  if(mainSqlJsonObject==null)
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			new SnippetException(dataPipeline,"Failed to execute deleteCredential", new Exception(e));
		}
	}
}
