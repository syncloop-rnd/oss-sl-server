package packages.middleware.pub.environments.adapter.services.DQL;
...
public final class getVpcSubnetByEnvironmentId {
		...
		try{
		  if(mainSqlJsonObject==null)
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/environments/adapter/services/DQL/getVpcSubnetByEnvironmentId.sql";
			  if(mainSqlJsonObject==null)
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			new SnippetException(dataPipeline,"Failed to execute getVpcSubnetByEnvironmentId", new Exception(e));
		}
	}
}
