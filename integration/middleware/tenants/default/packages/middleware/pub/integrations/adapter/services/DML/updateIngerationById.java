package packages.middleware.pub.integrations.adapter.services.DML;
...
public final class updateIngerationById {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null)
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/integrations/adapter/services/DML/updateIngerationById.sql";
			  if(mainSqlJsonObject==null){
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute updateIngerationById", new Exception(e));
		}
	}
}
