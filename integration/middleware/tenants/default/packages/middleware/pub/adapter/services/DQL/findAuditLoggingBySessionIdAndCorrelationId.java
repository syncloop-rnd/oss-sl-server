package packages.middleware.pub.adapter.services.DQL;
...
public final class findAuditLoggingBySessionIdAndCorrelationId {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/adapter/services/DQL/findAuditLoggingBySessionIdAndCorrelationId.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute findAuditLoggingBySessionIdAndCorrelationId", new Exception(e));
		}
	}
}
