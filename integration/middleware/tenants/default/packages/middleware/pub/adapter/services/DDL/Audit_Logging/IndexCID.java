		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/adapter/services/DDL/Audit_Logging/IndexCID.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute IndexCID", new Exception(e));
		}
	}
}
