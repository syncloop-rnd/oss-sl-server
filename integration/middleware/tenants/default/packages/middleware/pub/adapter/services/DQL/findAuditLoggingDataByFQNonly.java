...
public final class findAuditLoggingDataByFQNonly {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/adapter/services/DQL/findAuditLoggingDataByFQNonly.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute findAuditLoggingDataByFQNonly", new Exception(e));
		}
	}
}
