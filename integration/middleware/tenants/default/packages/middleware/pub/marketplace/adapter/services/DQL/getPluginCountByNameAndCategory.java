package packages.middleware.pub.marketplace.adapter.services.DQL;
...
public final class getPluginCountByNameAndCategory {
		...
		try{
		  ...
		  if(mainSqlJsonObject==null || timeout==0)//-----reset fix
			synchronized(syncBlock){
			  ...
			  String flowRef = location+"packages/middleware/pub/marketplace/adapter/services/DQL/getPluginCountByNameAndCategory.sql";
			  if(mainSqlJsonObject==null || timeout==0){ //-----reset fix
		  ...
		  SqlResolver.execute(dataPipeline,mainSqlJsonObject);
			...
			throw new SnippetException(dataPipeline,"Failed to execute getPluginCountByNameAndCategory", new Exception(e));
		}
	}
}
