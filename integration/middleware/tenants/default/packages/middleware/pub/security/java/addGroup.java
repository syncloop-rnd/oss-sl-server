...
try{
    ...
    UserProfileManager.createGroupForTenant(groupName,dataPipeline);
  	dataPipeline.put("status", true);
  
  	dataPipeline.keyLog(groupName, "group-added");
}catch(Exception e){
    String groupName = dataPipeline.getString("groupName");
	dataPipeline.clear();
  	dataPipeline.put("error",e.getMessage());
  	dataPipeline.put("status", false);
    dataPipeline.keyLog(groupName, "adding-group-failed");
